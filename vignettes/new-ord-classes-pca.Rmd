...
The second method is eigenvalue decomposition (EVD) or eigendecomposition, used in `stats::princomp()` and `psych::principal()`. In EVD, we do not decompose $X$ itself but its covariance matrix $X^\top X$. To understand EVD in terms of SVD, again take $X=UDV^\top$. Then when we consider the covariance matrix, we have
